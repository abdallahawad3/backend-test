...
exports.createCashOrder = asyncHandler(async (req, res, next) => {
  ...
  if (!cart) return next(new ApiError(`No cart for user: ${req.user._id}`, 404));

  const cartPrice = cart.totalAfterDiscount && cart.totalCartPrice;
...
exports.updateOrderToPaid = asyncHandler(async (req, res, next) => {
  ...
  if (!order) return next(new ApiError(`No order with id: ${req.params.id}`, 404));
